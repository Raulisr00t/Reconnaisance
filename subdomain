...
def check_word(word, site):
    ...
    try:
        request = requests.get("https://"f"{word}.{site}", allow_redirects=False)
        if request.status_code < 400:
            with lock:
                subdomain_count += 1
                print("Subdomain Found! " + f'{word}.{site}')

    except KeyboardInterrupt:
        print("Process stopped forcly bu user..")
        sys.exit()

def check_words(site, words):
    with ThreadPoolExecutor(max_workers=500) as cpu:
        cpu.map(lambda word: check_word(word, site), words)
  ...
  \`,      `-.`7  7)   )      | |__) | | __ _ _ __   ___| |_| |
   \/         \|  \'  / `->   |  ___/| |/ _` | '_ \ / _ \ __| |
        ...
        if resp.status_code < 400:
            for headers in resp.headers:
                print(headers,':',resp.headers[headers])
        else:
            pass

        original = site.split('://')[1]

        path = r'C:\Users\Student\Desktop\dnsss.txt'
